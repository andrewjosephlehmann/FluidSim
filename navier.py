...
def main():
    ...
    for _ in tqdm(range(N_ITERATIONS)):
        ...
        d_p_next__d_x = central_difference_x(p_next) # type: ignore
        d_p_next__d_y = central_difference_y(p_next) # type: ignore
        ...
        u_prev = u_next
        v_prev = v_next
        p_prev = p_next # type: ignore
    

    # The [::2, ::2] selects only every second entry (less cluttering plot)
    plt.style.use("dark_background")
    plt.figure()
    plt.contourf(X[::2, ::2], Y[::2, ::2], p_next[::2, ::2], cmap="coolwarm") # type: ignore
    plt.colorbar()

    plt.quiver(X[::2, ::2], Y[::2, ::2], u_next[::2, ::2], v_next[::2, ::2], color="black") # type: ignore
